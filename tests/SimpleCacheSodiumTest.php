...
//        $this->assertEquals($value, $storedValue);
    ...
    {
        $this->testSodiumNotStrict->setDefaultSeconds(43200);
        for ($i=0; $i<250; $i++) {
            $key = 'TestKey' . $i;
            $val = 'TestVal' . $i;
            $this->testSodiumNotStrict->set($key, $val, -1);
        }
        $below = 0;
        $above = 0;
        $exact = 0;
        foreach ($this->testSodiumNotStrict->fakeCache as $key => $value) {
            $ttl = $value->ttl;
            if ($ttl === 43200) {
                $exact++;
            }
    ...
    {
        $this->testSodiumNotStrict->setDefaultSeconds(432);
        for ($i=0; $i<250; $i++) {
            $key = 'TestKey' . $i;
            $val = 'TestVal' . $i;
            $this->testSodiumNotStrict->set($key, $val, -1);
